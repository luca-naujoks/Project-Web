...
export default function Page() {
  useEffect(() => {
    ...
    function g(t) {
      t.forEach((e) => {
        const r = `<tr class="hover:bg-gray-50">
        ...
        <div class="relative h-10 w-10">
          ...
          <span
            class="absolute right-0 bottom-0 h-2 w-2 rounded-full ${
              e.available ? "bg-emerald-400" : "bg-red-400"
            } ring ring-white"
          ></span>
        </div>
        <div class="text-sm">
      ...
      <td class="px-6 py-4">
        ...
        document
          ...
          ?.insertAdjacentHTML("beforebegin", r);
      });
    }
    function p(t, e) {
      let r = (t.rentingPeriod / 60) * t.pricePerMin,
        l = e * t.pricePerKm;
      return r > l ? l : r;
    }
    function f(t) {
      let e = 0,
        r = 0;
      return setInterval(() => {
        e++, t.rentingPeriod++, e === 60 && ((e = 0), r++);
        const i =
          r.toString().padStart(2, "0") + ":" + e.toString().padStart(2, "0");
        (t.rentingTimer = i),
          (document.querySelector(`#scooter-timer-${t.id}`).innerHTML = i);
      }, 1e3);
    }
    function b(t) {
      document.querySelector(`#scooter-timer-${t}`).innerHTML = "00:00";
    }
    function s(t) {
      var e, r;
      (e = document.querySelector(`[data-scooter-available-${t}]`)) == null ||
        e.classList.toggle("hidden"),
        (r = document.querySelector(`[data-scooter-booked-${t}]`)) == null ||
          r.classList.toggle("hidden");
    }
    function u(t) {
      var e;
      (e = document.querySelector(
        `[data-type="book"][data-scooterid="${t}"]`
      )) == null || e.classList.toggle("hidden"),
        document
          .querySelector(`[data-type="return"][data-scooterid="${t}"]`)
          .classList.toggle("hidden");
    }
    let c = 0,
      o = document.getElementById("distance");
    const a = [
      {
        id: 1,
        name: "E-Scooter 1",
    ...
    function v(t) {
      const e = a.filter((l) => l.id == t)[0];
      e.available = !1;
      const r = f(e);
      (e.interval = r), s(t), u(t);
    }
    function m(t) {
      if (o.value == "") {
    ...
    function y() {
      ...
      const t = a.filter((e) => e.available === !1);
      for (const e of t) m(e.id);
    }
    g(a);
    document.querySelectorAll(".book-scooter").forEach((t) => {
      t.addEventListener("click", (e) => {
        const r = e.target.getAttribute("data-scooterid");
        v(r);
      });
    });
    document.querySelectorAll(".return-scooter").forEach((t) => {
      t.addEventListener("click", (e) => {
        const r = e.target.getAttribute("data-scooterid");
        m(r);
      });
    ...
    <div class="m-5">
      <div class="mx-auto max-w-screen-lg">
        <h1 class="text-center mt-10 text-3xl font-extrabold text-gray-90 md:text-5xl lg:text-6xl">
          <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-violet-400">
            ScooTeq GmbH
          </span>
        </h1>
          ...
          <table class="w-full border-collapse bg-black text-left text-sm text-gray-200">
            <thead class="bg-black">
              <tr>
                ...
                <th
                  scope="col"
                  class="flex px-6 py-4 font-medium text-gray-200"
                >
                  Ausleihstatus
                </th>
                <th scope="col" class="px-6 py-4 font-medium text-gray-200">
            ...
            <tbody
              class="divide-y divide-gray-100 border-t"
              id="table-body"
            ></tbody>
          </table>
        </div>
        ...
        <div class="button-container flex justify-between">
          <div>
            <label
              for="distance"
              class="block pl-1 text-sm font-medium text-gray-200"
            >
              Gesamtstrecke in km
            </label>
            <div class="mt-1">
        ...
        <div class="px-6 py-4 overflow-hidden rounded-lg border border-gray-900 shadow-md mt-10 flex justify-between font-bold text-lg text-gray-200">
